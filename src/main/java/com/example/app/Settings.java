 ...
 */
public class Settings implements RefreshSettings {
	...
	protected void btnAddFonction() {
		...
		if (newFonction != null && !newFonction.isEmpty()) {
			if (Fonction.getFonctionlist().contains(newFonction)) {
				...
				Fonction newFonctionObj = new Fonction(newFonction, vieworderPanel, neworderPanel, expensesPanel,
				...
				DatabaseConnection.addFonction(newFonction, vieworderPanel, neworderPanel, expensesPanel,
						settingsPanel, reportsPanel, menuitemsPanel);
				System.out.println(Fonction.getFonctionlist());
				addFonctioncb.getItems().clear();
				addFonctioncb.getItems().addAll(Fonction.getFonctionlist());
				fonctionUrcb.getItems().clear();
				fonctionUrcb.getItems().addAll(Fonction.getFonctionlist());
	...
	protected void AddRateonAction() {
		...
		try {
			...
			FXMLLoader fxmlLoader = new FXMLLoader(AddRate.class.getResource("Addrate.fxml"));
			...
			AddRate addExpenseController = fxmlLoader.getController();
	...
	protected void btnRemoveRateonAction() {
		...
		try {
			// Load the Settings.fxml file
			FXMLLoader fxmlLoader = new FXMLLoader(AddRate.class.getResource("Removerate.fxml"));
			Parent root = fxmlLoader.load();
			...
			RemoveRate RemoveRateController = fxmlLoader.getController();
